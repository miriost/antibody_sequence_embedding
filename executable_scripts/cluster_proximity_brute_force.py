...
def get_distance_map(data, cpus=2):
    distances_map = pairwise_distances(X=data, metric='euclidean', n_jobs=cpus)
    return distances_map


@ray.remote
def get_knn(data, k):
    return np.argpartition(data, k, axis=1)[:, 0:k]


def get_knn_map(distance_map, cluster_size, cpus=2):
    # parallel getting the proximity map
    step = distance_map.shape[0]/cpus
    ranges = [[round(step*i), round(step*(i+1))] for i in range(cpus)]
    results_ids = []
    for x in ranges:
        results_ids += [get_knn.remote(distance_map[x[0]:x[1], :], cluster_size+1)]
    proximity_map = np.zeros(shape=[distance_map.shape[0], cluster_size+1], dtype=int)
    for i, x in enumerate(ranges):
        res = ray.get(results_ids[i])
        proximity_map[x[0]:x[1], :] = res

    return pd.DataFrame(proximity_map)


def test_build_dist_and_knn_maps(dim_one=100, dim_two=10, cluster_size=3, cpus=2):
    df = pd.DataFrame(np.random.rand(dim_one, dim_two))
    t0 = time.time()
    distance_map = get_distance_map(df, cpus)
    print("distance map completed after {}".format(time.time() - t0))
    t0 = time.time()
    knn_map = get_knn_map(distance_map, cluster_size, cpus)
    print("knn map completed after {}".format(time.time() - t0))
...
def cluster(data_file, output_file_path, output_file_name, cluster_size=100, cpus=2):
    ...
    t0 = time.time()
    distance_map = get_distance_map(vectors, cpus)
    print("distance map completed after {}".format(time.time() - t0))
