...
def main(argv):
   ...
   if fasta_type and not fasta_file:
       ...
       pv = ProtVec(corpus_fname=fasta_file, corpus=None, n=n_len, reading_frame=reading_frame, trim = trim_range,size=100, out=output_corpus_file, sg=1, window=25, min_count=2, workers=3)
       ...
       pv = ProtVec(corpus_fname=None, corpus=corpus_object, n=n_len, reading_frame=reading_frame, trim = trim_range, size=100, out=output_corpus_file, sg=1, window=25, min_count=2, workers=3)
       process_time = (time.time() - t0)/60 # in minutes
       print('Model built in {} minutes, saving...'.format(process_time))
