...
usage="USAGE: run_classification.sh -f [list of fold numbers] -c [list of cluster sizes] -s [list of segnificant levels] -m [list of min subjects] -o [output file name]"
folds=$(seq 0 1 39)
	...
	for cs in ${cluster_sizes}; do
		...
		for seg_level in ${segnificant_levels} ; do
			...
			for min_subj in ${min_subjects}; do 
				...
				python -u ~/antibody_sequence_embedding/executable_scripts/classify_no_splitting.py --train_file ${output_dir}/train_feature_table.csv  --test_file ${output_dir}/test_feature_table.csv --col_names="min_subj,fold,cluster_size,segnificant" --col_values="${min_subj},${fold},${cs},${sig}" --output_file ${output_dir}/${output_file} -M all 2>&1 | tee ${output_dir}/classifiy_no_splitting.log.txt
