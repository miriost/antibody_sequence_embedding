	...
	for knn_itr in ${knn}; do
      ...
      for min_significance_itr in ${min_significance}; do
        ...
        for max_distance_percentil_itr in ${max_distance_percentil}; do
          ...
          cmd="nice -19 python -u ~/antibody_sequence_embedding/classify_repertoire.py ${max_distance_percentil_dir}/selected_train_feature_table.csv
          ${max_distance_percentil_dir}/selected_test_feature_table.csv --col_names=\"knn,min_subjects,min_significance,max_distance_percentil\" --col_values=\"${knn_itr},${min_subjects_itr},
