...
if [ $MIN_STEP -le 0 ] && [ 0 -le $MAX_STEP ]; then
	...
	#python -u ~/miris_tools/reprocess_makedb_dir.py ${DIR}/data ${DIR}/data/${NAME}_aa_trim_3_4.csv --trim 3 4 2>&1 | tee ${DIR}/logs/reprocess_makedb_dir.log.txt ;
	python -u ~/miris_tools/reprocess_makedb_dir.py ${DIR}/data ${DIR}/data/${NAME}_aa_trim_3_4.csv --trim 0 4 2>&1 | tee ${DIR}/logs/reprocess_makedb_dir.log.txt ;
	...
	python -u ~/miris_tools/db_to_cdr3_fasta.py -i ${DIR}/data/${NAME}_aa_trim_3_4.csv -o ${DIR}/${NAME}_cdr3_aa_trim_3_4.fasta -c JUNC_AA ; 
fi
...
if [ $MIN_STEP -le 1 ] && [ 1 -le $MAX_STEP ]; then
	...
	for p in 1.0; do
		pid = python -u ~/miris_tools/executable_scripts/generate_model.py -f ${DIR}/${NAME}_cdr3_aa_trim_3_4.fasta -o ${DIR}/models/${NAME}_${p} -p ${p} 2>&1 | tee -a ${DIR}/logs/generate_model.log.txt &  
	done ;
fi
...
if [ $MIN_STEP -le 2 ] && [ 2 -le $MAX_STEP ]; then
	...
	for model in ${DIR}/models/*.model ; do 
		python -u ~/miris_tools/executable_scripts/generate_vectors.py ${DIR}/data/${NAME}_aa_trim_3_4.csv -o ${DIR} -c JUNC_AA ${model} 2>&1 | tee -a ${DIR}/logs/generate_vectors.log.txt & 
	done ;
fi
...
if [ $MIN_STEP -le 3 ] && [ 3 -le $MAX_STEP ]; then
	...
	for vector_file in $(ls ${DIR}/filtered_data_sets/); do 
		Rscript ~/miris_tools/V_family_labels.R ${DIR}/filtered_data_sets/${vector_file} ${DIR}/vfamily_labels/$(echo "${vector_file}" '\t' | sed 's/FILTERED_DATA.csv/labels.csv/g') &
	done ;
fi
...
if [ $MIN_STEP -le 4 ] && [ 4 -le $MAX_STEP ]; then
	...
	for vector_file in $(ls ${DIR}/vectors/) ; do 
		python -u ~/miris_tools/executable_scripts/reduce_dimensions.py -i ${DIR}/vectors/${vector_file} -o ${DIR}/vectors_reduced_to_10dim/${vector_file} -n 10 -m PCA 2>&1 | tee -a ${DIR}/logs/reduce_dimensions.log.txt & 
	done ;
fi
...
if [ $MIN_STEP -le 5 ] && [ 5 -le $MAX_STEP ]; then
	...
	for vector_file in $(ls ${DIR}/vectors_reduced_to_10dim/) ; do 
		python -u ~/miris_tools/V_family_analysis.py ${DIR}/vectors_reduced_to_10dim/${vector_file} ${DIR}/vfamily_labels/$(echo "${vector_file}" | sed 's/VECTORS.csv/labels.csv/g') -l V_FAMILY 2>&1 | tee -a ${DIR}/logs/V_family_analysis.log.txt & 
	done ;
fi
