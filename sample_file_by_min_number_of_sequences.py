...
for subject, frame in by_subject:
    ...
    if len(frame) < min_num_smaples_per_subject:
        print('low number of sequences, subject removed.\n')
    else:
        positive_subjects += 1
        sampled_frame_indexes = np.random.choice(frame.index, min_num_smaples_per_subject, replace = False)
        ...
        print('Subject sampled and added.')
        
        
    
print('----- Summary -----')
print(f'Analysed {sub_num!r} subjects')
print(f'Chose {positive_subjects!r} subjects with > {min_num_smaples_per_subject} number of sequences')
